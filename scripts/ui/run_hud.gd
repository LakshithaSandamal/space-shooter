...
var _time_warp_label: Label
var _overcharge_label: Label
...
func set_combo_state(multiplier: int, grace_ratio: float) -> void:
	...
	if _combo_label != null:
		...
		var combo_color: Color = StarfallVisualTokens.color(&"gold_bright") if _combo_multiplier > 1 else StarfallVisualTokens.color(&"text_muted")
		_combo_label.add_theme_color_override("font_color", combo_color)
	queue_redraw()

func set_power_up_state(
	shield_active: bool,
	_shield_ratio: float,
	time_warp_active: bool,
	time_warp_ratio: float,
	overcharge_active: bool,
	overcharge_ratio: float
) -> void:
	if _shield_label != null:
		_shield_label.visible = shield_active
		_shield_label.text = "SHIELD  1"
	if _time_warp_label != null:
		_time_warp_label.visible = time_warp_active
		_time_warp_label.text = "WARP  %02d%%" % int(round(time_warp_ratio * 100.0))
	if _overcharge_label != null:
		_overcharge_label.visible = overcharge_active
		_overcharge_label.text = "OVER x2  %02d%%" % int(round(overcharge_ratio * 100.0))

func show_power_up_feedback(power_up_type: int) -> void:
	match power_up_type:
		StarfallPowerUpSystem.PowerUpType.SHIELD:
			_show_feedback("SHIELD ONLINE", StarfallVisualTokens.color(&"cyan_soft"))
		StarfallPowerUpSystem.PowerUpType.TIME_WARP:
			_show_feedback("TIME WARP", StarfallVisualTokens.color(&"purple_bright"))
		StarfallPowerUpSystem.PowerUpType.OVERCHARGE:
			_show_feedback("OVERCHARGE x2", StarfallVisualTokens.color(&"magenta_primary"))

func show_shield_save() -> void:
	_show_feedback("SHIELD SAVE", StarfallVisualTokens.color(&"cyan_primary"))
...
func show_combo_break(previous_multiplier: int) -> void:
	if previous_multiplier <= 1:
		return
	_show_feedback("COMBO LOST  x%d" % previous_multiplier, StarfallVisualTokens.color(&"magenta_primary"))
...
func reset_run() -> void:
	...
	if _time_warp_label != null:
		_time_warp_label.visible = false
	if _overcharge_label != null:
		_overcharge_label.visible = false
	set_combo_state(1, 0.0)

func show_game_over(distance_m: float, core_count: int, score: int, best_combo: int, near_miss_count: int) -> void:
	if _game_over_panel == null:
		return
	_game_over_stats.text = "%d m   •   %d CORES   •   %d SCORE\nBEST COMBO x%d   •   %d NEAR MISSES" % [int(floor(distance_m)), core_count, score, best_combo, near_miss_count]
...
func _draw() -> void:
	...
	draw_line(combo_bar_origin, combo_bar_origin + Vector2(combo_bar_width, 0.0), StarfallVisualTokens.alpha(&"line_dark", 0.76), 3.0, true)
	if _combo_multiplier > 1 and _combo_grace_ratio > 0.0:
		...
		draw_line(combo_bar_origin, combo_bar_origin + Vector2(combo_bar_width * _combo_grace_ratio, 0.0), grace_color, 3.0, true)
...
func _build_labels() -> void:
	...
	_shield_label = _make_power_label("SHIELD  1", StarfallVisualTokens.color(&"cyan_soft"))
	_time_warp_label = _make_power_label("WARP  100%", StarfallVisualTokens.color(&"purple_bright"))
	_overcharge_label = _make_power_label("OVER x2  100%", StarfallVisualTokens.color(&"magenta_primary"))
...
func _make_power_label(text_value: String, color: Color) -> Label:
	var label := Label.new()
...
func _layout_controls() -> void:
	...
	_shield_label.position = Vector2(24.0, 100.0)
	_shield_label.size = Vector2(104.0, 22.0)
	_time_warp_label.position = Vector2(132.0, 100.0)
	_time_warp_label.size = Vector2(116.0, 22.0)
	_overcharge_label.position = Vector2(252.0, 100.0)
	_overcharge_label.size = Vector2(142.0, 22.0)
	_skill_feedback_label.position = Vector2(maxf(0.0, size.x * 0.5 - 220.0), 136.0)
